...
def test_complete_workflow_openai(openai_agent):
    ...
    discount_request = next(
        (msg for msg in state["messages"] if msg["type"] == "discount_proof_request"),
        None
    )
    ...
    quote_msg = next(
        (msg for msg in state["messages"] if msg["type"] == "quote"),
        None
    )
    ...
    review_msg = next(
        (msg for msg in state["messages"] if msg["type"] == "review_summary"),
        None
    )
